...
for ((i=0;i<10;i++))
do
  DB_CONNECTABLE=$(mysql -uroot -p${MYSQL_ROOT_PASSWORD} -hdb -e 'status' >/dev/null 2>&1; echo "$?")
...
if [[ $DB_CONNECTABLE -eq 0 ]]; then
  DB_EXISTS=$(mysql -uroot -p${MYSQL_ROOT_PASSWORD} -hdb -e "SHOW DATABASES LIKE '"known"';" 2>&1 |grep "known" > /dev/null ; echo "$?")

  if [[ DB_EXISTS -eq 1 ]]; then
    echo "=> Creating database known"
    RET=$(mysql -uroot -p${MYSQL_ROOT_PASSWORD} -hdb -e "CREATE DATABASE known")
    if [[ RET -ne 0 ]]; then
      echo "Cannot create database for known"
      exit RET
    fi
    echo "=> Loading initial database data to known"
    RET=$(mysql -uroot -p${MYSQL_ROOT_PASSWORD} -hdb known < /var/www/html/schemas/mysql/mysql.sql)
    if [[ RET -ne 0 ]]; then
...
echo "dbpass = '${MYSQL_ROOT_PASSWORD}'"       >> /var/www/html/config.ini
